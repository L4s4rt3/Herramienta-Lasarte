...
| Revisión del parte antes del correo | ✅ | Desde 11-09: cada mañana, ANTES de enviar el aviso, se revisa el parte de ayer y los 14 días de la ventana (`scripts/lib-revision-parte.mjs`). Nueve comprobaciones: que exista el parte, que haya producción, que estén los informes de lote y el GSTOCK, que esté analizado, que cuadre con su detalle, que no haya pasadas repetidas ni palets de más de 10 t, y que los cinco del papel estén puestos (tecleados **o** estimados). **Y cada día se arregla y se cuadra lo que se pueda**, en este orden: (1) se quitan las pasadas contadas dos veces —solo la fila del volcado, solo si su gemela del parte tiene todo lo que ella tiene y no cuelga nada de ella; la fila entera se guarda en la propia marca de revisión antes de borrarla, y lo que se llevaría algo por delante NO se toca y sale en el correo—; (2) se rehace el parte (remonta informes y reanaliza) si el detalle no cuadra o falta el análisis. Ninguna de las dos toca un parte Validado. Lo que no se puede arreglar, el correo lo explica con **una valoración de qué se cree que ha pasado**. La marca queda en `partes_diarios.revision` y se ve en la app encima de los KPI. **El descuadre (DSJ) NO bloquea ni cuenta como reparo**: medido sobre 42 días, 26 pasan del ±5 % porque lo de un día se paletiza al siguiente — se diagnostica, no se convierte en alarma. "Validado" sigue siendo el único candado humano. |
...
| Análisis ad hoc para dirección (tipo stock S33-S34) | Luis con scripts | **HECHO 03-09 para el más pedido**: la pestaña Entradas → «Campaña» (pérdida y Mercadona por productor y finca) sustituye al Excel `analisis-mermas-mercadona.ts`, que ahora usa las mismas funciones (`_shared/mermaMdnaAgregado.ts`, `_shared/mdnaMix.ts`). El aprovechamiento REAL por parcela (Invermarmelo) también es vista desde el 03-09 (Análisis → Por productor → «Aprovechamiento real por parcela»). El económico por tipo de día también (03-09, Económico → Rentabilidad → «Por tipo de día»). Los tres análisis recurrentes son ya vistas de la app con las mismas funciones que sus scripts. |
...
- ✅ **HECHO 16-09** — **Análisis completo de la empresa cada lunes para José María** (`scripts/analisis-semanal-empresa.mjs`, tarea de Windows «Lasarte - Analisis semanal», lunes 05:45, correo a soporte@ con Excel de 15 hojas: Resumen, Para José María, Producción por día, Calibrador, Cuadre, Calidad al calibrar, Camiones y entradas cruzados con las hojas PESO y los Laadbon de la LAN, Calidad de entrada, Stock, Ventas, Cuenta, Personas, Pendientes y sistema, Notas). Cada cifra lleva su método y su fuente; lo que solo existe en papel (horas de RRHH, notas del chat y cuaderno, recuento de cámara) se lee si alguien lo deja en su sitio y si no va como SIN DATO. Nació de la semana 37, que se hizo a mano en siete versiones. **Regla del ERP del mismo día** (tras una consulta nuestra que colgó el servidor a toda la oficina): de 06:00 a 15:00 el ERP intocable, nada de consultas grandes, `conectarErp()` fija `max_execution_time` 120 s, y la tarea «Lasarte - Vigilar ERP» (`scripts/vigilar-erp-consultas.mjs`, cada hora de 06:05 a 15:05) cancela las consultas nuestras de más de 60 s y avisa de las ajenas.
...
- ✅ **HECHO 02-09 (tarde)** — **Automatizaciones de la auditoría**:
  ...
  - *Pestaña Entradas → «Campaña»* (03-09): pérdida (merma de cámara, podrido de tría, podrido de calibrador, €) y Mercadona por formato, por productor y por finca, con el podrido por mes y los lotes apartados. Era el Excel `analisis-mermas-mercadona.ts` regenerado 5 veces en 3 semanas. Las funciones son las mismas para la pestaña y el script (`_shared/mermaMdnaAgregado.ts`, `_shared/mdnaMix.ts`: el mix de Mercadona estaba triplicado con dos criterios de "clase apta"); el script reescrito da el Excel idéntico al anterior (comparado celda a celda). El mix por lote viene de la vista materializada `clasificacion_lote_mix_mv` (refresco horario por pg_cron; 260.000 filas de la vista canónica → 20.940 pivotadas → 1,9 MB en una RPC). Diferencias con el Excel: la pestaña no simula la campaña cerrada (los lotes abiertos se listan) y el mix sale de la vista canónica del calibrador, no de la tabla del Excel manual.
  - *Vista Análisis → Por productor → «Aprovechamiento real por parcela»* (03-09): el aprovechamiento MEDIDO (no estimado) de cualquier finca y parcela, que era el Excel `informe-aprovechamiento-invermarmelo.ts` clavado a dos parcelas. Mismas funciones que el script (`_shared/aprovechamientoReal.ts`; Excel comparado celda a celda). Lee la RPC `clasificacion_detalle_lotes(lotes)` sobre la materializada `clasificacion_lote_detalle_mv` (la vista canónica al grano de pasada × producto × clase × calibre, 273.000 filas, índice por lote, refresco horario con el mix). Enseña hasta qué día llega cada fuente y avisa si el volcado va por detrás de los partes. Al hacerla se destaparon y arreglaron dos cosas en la base (migración `20260903110937`): (1) la rama «parte» de `clasificacion_lote` solo se apartaba del volcado por código CRUDO, así que "26041406+3 BOX" entraba dos veces: las 33.806 filas que aportaba (2,76 M kg, 152 lotes) eran todas duplicados del mismo lote y día, y ahora la regla es por lote y día como en la rama del Word; (2) el volcado SQL escribe la clase SIN letra ("Extra 1 ") y el mix materializado solo miraba la letra, con lo que el 87 % de las filas salían sin clase apta. `public.clase_letra()` / `letraClase()` deducen la letra del nombre con la misma tabla en SQL y en TypeScript.
  - *Las dos cargas que estaban paradas, automatizadas* (04-09): hacer las tres vistas destapó que la asistencia llevaba sin volcarse desde el 04-08 y que la facturación de Mercadona se quedó en la semana 32, un mes las dos, sin que nada avisara. (1) **Tarifa Mercadona desde el ERP** (`scripts/mercadona-facturacion-erp.mjs`, dentro de la tarea diaria del ERP): lee `fact_lin_alb` del cliente Mercadona en solo lectura, ata el formato por `palets_cab.formula_confeccion` y agrupa por semana ISO con la fecha de ALBARÁN; validado al céntimo contra las semanas 30 y 31, que ya estaban bien facturadas. Cargó las semanas 21-32 que faltaban (+1,29 M€ de base) y solo escribe `base_iva`/`lineas`. (2) **Asistencia desde el reloj** (`scripts/importar-asistencia-reloj.mjs`, tarea los lunes 09:30): presente = 1 hora o más, no pisa lo tecleado a mano, no crea trabajadores; cargó 16 días de agosto y reproduce exactos los días que ya estaban tecleados. (3) **El vigía las vigila**: reglas `asistencia-parada` y `mercadona-sin-facturar`. (4) **El estándar de kg/persona se edita en la app** (tabla `estandar_rendimiento` + historial; el JSON de los informes en Python pasa a espejo que regenera `scripts/sincronizar-estandar.mjs`). (5) Las tres vistas exportan su Excel con un botón.
  - *Vista Económico → Rentabilidad → «Por tipo de día»* (03-09): plantilla completa/reducida × día bueno/medio/malo, con la estructura (kg, personas, personal €/kg) de todos los días clasificados y la cuenta entera (v5) solo en los días con tarifa Mercadona real (semanas con ≥0,80 €/kg facturados). Era `tmp/analisis-tipo-dia.ts`, un Excel FUERA del repo que dependía de dos ficheros locales (el estándar y un export del reloj). Ahora: el estándar por régimen vive en `_shared/estandarRendimiento.ts` (una sola fuente para la app, el vigía y los scripts; un test lo mantiene igual que `scripts/informe-produccion/estandar.json`, que siguen leyendo los informes en Python y el correo diario); el cálculo en `_shared/tipoDia.ts` sobre `computeRentabilidadDia`; el script tracked `scripts/analisis-tipo-dia.ts` da el Excel con las mismas funciones (`--fuente=tabla` reproduce la versión vieja para comparar). Los kg salen de la RPC `rentabilidad_filas_dias(desde, hasta)` sobre la materializada de detalle (lote × producto × clase por día). Lo que no hay en la base no se inventa: los días sin asistencia (desde el 05-08, el volcado semanal está parado) se listan y entran solos cuando se vuelque; el régimen reducido de agosto solo se verá entonces. De paso el vigía tenía el borde al revés (35 presentes era completa; el dueño dice ≤35 reducida): arreglado al compartir la fuente.
