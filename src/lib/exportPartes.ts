...
export function exportPartesToExcel(partes: ParteRow[], from: string, to: string) {
  ...
  const resumenAOA: (string | number)[][] = [
    ...
    ["RESUMEN GLOBAL"],
    ["Indicador", "Valor"],
    ["N de partes", partes.length],
    ["Produccion real total (kg)", +totalProd.toFixed(2)],
    ...
    ["Semáforo", "Nº partes", "%"],
    ["Verde (< 3%)", nOk,       +(partes.length ? nOk / partes.length * 100 : 0).toFixed(1)],
    ["Amarillo (3-5%)", nAmarillo, +(partes.length ? nAmarillo / partes.length * 100 : 0).toFixed(1)],
    ["Rojo (> 5%)", nAlerta,   +(partes.length ? nAlerta / partes.length * 100 : 0).toFixed(1)],
    [],
  ...
  const detalleRows = enriched.map(({ p, c }) => [
    ...
    +c.dsj.toFixed(2),
    +c.dsj_pct.toFixed(3),
    c.semaforo === "verde" ? "OK" : c.semaforo === "amarillo" ? "Revisar" : "Critico",
    p.notas_generales ?? "",
    p.notas_inventario ?? "",
  ]);
...
function semLabel(s: "verde" | "amarillo" | "rojo"): string {
  return s === "verde" ? "OK < 3%" : s === "amarillo" ? "Revisar 3-5%" : "Critico > 5%";
}
  ...
  autoTable(doc, {
    ...
    didParseCell: (data) => {
      ...
      if (data.column.index === 10 && data.section === "body") {
        ...
        else if (v.startsWith("Revisar")) data.cell.styles.textColor = C_SEM_WARN;
        else if (v.startsWith("Critico")) data.cell.styles.textColor = C_SEM_ERROR;
      }
